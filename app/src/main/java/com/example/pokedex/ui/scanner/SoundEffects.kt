...
enum class AppSoundEffect(
    ...
    val fileStem: String,
    @param:RawRes val defaultResource: Int,
) {
    ScanStarted("开始扫描", "Scan started", "scan_started", R.raw.scan_start),
    RecognitionSuccess("识别成功", "Recognition success", "recognition_success", R.raw.recognition_success),
    RecognitionFailure("识别失败", "Recognition failure", "recognition_failure", R.raw.recognition_failure),
}
...
sealed interface ResolvedSoundAsset {
    data class BuiltIn(@param:RawRes val resourceId: Int) : ResolvedSoundAsset
...
class SoundEffectStore(private val context: Context) {
    ...
    fun resolve(effect: AppSoundEffect): ResolvedSoundAsset =
        customFile(effect)?.let(ResolvedSoundAsset::Custom)
            ?: ResolvedSoundAsset.BuiltIn(effect.defaultResource)
        ...
        runCatching {
            MediaPlayer().apply {
                ...
                when (val asset = store.resolve(effect)) {
                    ...
                    is ResolvedSoundAsset.BuiltIn -> appContext.resources.openRawResourceFd(asset.resourceId).use { descriptor ->
                        requireNotNull(descriptor) { "无法读取内置音效" }
